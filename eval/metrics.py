...
def brier_score(y_true: list[int], y_pred_proba: list[float]) -> float:
    ...
    mse = sum((yt - yp) ** 2 for yt, yp in zip(y_true, y_pred_proba)) / len(y_true)
    return mse
...
def accuracy_score(y_true: list[int], y_pred_proba: list[float], threshold: float = 0.5) -> float:
    ...
    correct = sum(yt == yp for yt, yp in zip(y_true, y_pred_binary))
    return correct / len(y_true)


def precision_recall_f1(y_true: list[int], y_pred_proba: list[float], threshold: float = 0.5) -> tuple[float, float, float]:
...
def calibration_curve(y_true: list[int], y_pred_proba: list[float], n_bins: int = 10) -> list[dict]:
    ...
    sorted_pairs = sorted(zip(y_pred_proba, y_true))
